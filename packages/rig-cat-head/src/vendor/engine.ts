...
export interface BotFrame {
  ...
  attachmentSurface: { sil: Silhouette; gaze: { yaw: number; pitch: number; roll: number }; radius: number; visibility: number }
  attachmentYaw: number
...
function blendPose(a: Pose, b: Pose, t: number): Pose {
  ...
  return {
    ...
    headTurn: lerp(a.headTurn ?? 0,b.headTurn ?? 0,t),
...
export class BotEngine {
  ...
  /** Mofli adapter addition: fork dated state without sharing the mutable point scratch buffer. */
  fork(): BotEngine {
    const copy = Object.assign(new BotEngine(this.scale), this)
    copy.pts = []
    return copy
  }
  ...
  setExpression(expression: BotExpression | null, now = 0) {
    ...
    this.exprPrev = this.exprAtTime(now) ?? EXPRESSIONS[0]!
    this.expr = expression
    this.exprAt = now
  }
  ...
  setShape(radii: number[] | null, now = 0) {
    ...
    this.shapePrev = this.shapeAtTime(now)
    this.shape = radii
    this.shapeAt = now
  }
  ...
  private posed(
    ...
    // The selected master carries its ears in the same contour as the face.
    // Other head forms inherit a reduced ear displacement before pose blending.
    const earMix: Record<string, number> = { egg: .8, hexagon: .5, play: .3, orbit: .3, comet: .15 };
    if (!def.baseBody && shape && earMix[def.id]) {
  ...
  setState(id: StateId, now: number) {
    ...
    // Preserve an in-flight blink when a state change interrupts it.
  ...
  sample(now: number, interaction?: { look: { x: number; y: number }; attention: number; pressed: boolean; reaction: number }): BotFrame {
    ...
    const sil: Silhouette = {
      ...pose.sil,
      radii: turnedProfile(pose.sil.radii,gaze.yaw,gaze.pitch,pose.headTurn ?? 0),
      rot: pose.sil.rot + gaze.roll*Math.PI/180*(pose.headTurn ?? 0),
      cx: pose.sil.cx + offX,
      cy: pose.sil.cy + offY,
    ...
    return {
      attachmentPoints: this.pts.map(p => ({ ...p })),
      attachmentYaw: gaze.yaw,
      attachmentSurface: { sil, gaze, radius: R, visibility: pose.headTurn ?? 0 },
      bodyPath,
