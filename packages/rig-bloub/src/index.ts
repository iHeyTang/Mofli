...
function expression(skin: Skin) {
  ...
  if(design){
    const eyes=structuredClone(design.expressions[index+1]!);
    for(const eye of eyes)eye.lid=resolveEyeComponent(eye,skin.variants?.eyes==='socket');
    const limit=(v:number,n:number)=>Math.max(-n,Math.min(n,v)),socket=skin.variants?.eyes==='socket';
    return {...configured,eyes,character:design.motion,eyeCatalog:design.expressions,gaze:{yaw:limit(configured.gaze.yaw,socket?20:28),pitch:limit(configured.gaze.pitch,socket?18:26),roll:limit(configured.gaze.roll,socket?18:22)}};
  }
