...
test("each sequence links playback, thumbnails and seeking in both directions", async ({page}) => {
  ...
  await expect(page.locator('#expression-choices button[aria-pressed="true"]')).toHaveText('平静',{timeout:5000});
  await expect(page.locator('.transport .chip')).toHaveText('平静');
  await page.click('#play');
  await page.getByRole('button',{name:'喜欢',exact:true}).click();
  await expect(page.locator('.transport .chip')).toHaveText('喜欢');
  ...
  await page.getByRole('tab',{name:/动作/}).click();
  await page.locator('[data-state="notify"]').click();
  await expect(page.locator('.transport .chip')).toHaveText('消息提醒');
  ...
  await expect(page.locator('.transport .chip')).toHaveText('三角形');
  await page.locator('#seek input').focus();await page.keyboard.press('Home');
...
for (const viewport of [{width:390,height:844},{width:360,height:667},{width:768,height:1024}]) {
  test(`mobile five tabs keep preview visible at ${viewport.width}px`, async ({page}) => {
    ...
    await page.getByRole('button',{name:'喜欢',exact:true}).click();
    await expect(page.getByRole('button',{name:'喜欢',exact:true})).toHaveAttribute('aria-pressed','true');
