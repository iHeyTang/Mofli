...
- Package direction is skin -> exactly one rig -> core. Core must not import rigs/skins; rigs must not import skins, other rigs, or studio. Use public package imports across boundaries, never sibling source paths. Each package must build independently with installed declared dependencies.
